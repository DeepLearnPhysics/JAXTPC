...
#     ./slurm/run_doraemon_wire.sh                     # submit full set
#     START=300 STOP=500 ./slurm/run_doraemon_wire.sh  # only files 300:500
...
#     ./slurm/run_doraemon_wire.sh status              # report done/missing
...
# --- readout-specific (the ONLY block that differs from the pixel script) ---
...
WORKERS=12          # save workers. <=12 on a 40GB GPU (24 OOMs the card —
                    ...
                    # memory/run_batch-worker-ceiling).
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Readout     : wire    outdir $OUTDIR"
...
echo "============================================================"
echo " WIRE array task $b : files ${RANGE_START}:${RANGE_STOP}  shard-id ${SHARD}"
