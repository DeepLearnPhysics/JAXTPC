...
Serves production HDF5 files (step/hits/sensor, plus the optional labl) and
...
(`production_run/{step,hits,sensor}/`, plus optional `production_run/labl/`).
...
(auto-detected next to the step path). If no labl file is found, only the
...
# Custom settings
python3 viewer/export_gif.py path/to/sim_step_0000.h5 \
    --event 0 \
    --duration 6 \
    --fps 30 \
    --size 1080 1080 \
...
python3 viewer/export_gif.py path/to/sim_step_0000.h5 -e 0 -v 0 --light

# MP4 output (requires ffmpeg)
python3 viewer/export_gif.py path/to/sim_step_0000.h5 -e 0 -o event.mp4
```
