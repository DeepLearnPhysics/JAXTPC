"""
Loss and gradient surfaces for detector physics parameters.

Sweeps each detector parameter (velocity, lifetime, recomb alpha, recomb beta_90)
while holding the muon track fixed at truth. Shows that the simulation is
differentiable through these parameters and gradients point toward truth.

Uses the Sobolev geomean log1p loss (s=1.5) — same as the muon optimization.

Run from project root:
    python3 closure/muon/detector_surfaces.py
"""
...
# Detector parameter sweep configs: (range, truth_value, unit, fd_epsilon)
DETECTOR_PARAMS = {
    'diffusion_trans': {
...
def main():
    print("=" * 60)
    print("DETECTOR PARAMETER GRADIENT SURFACES")
    print("=" * 60)
    ...
    print(f"Muon: {N_SEGMENTS} segments, {int((de > 0).sum())} active\n")
    ...
    for param_name, cfg in DETECTOR_PARAMS.items():
        ...
        fig.suptitle(
            ...
            f'Sobolev geomean log1p',
            fontsize=14, fontweight='bold',
        )
        fig.tight_layout()
        fname = os.path.join(OUT_DIR, f'detector_surface_{param_name}.png')
