...
Usage:
    python3 closure/muon/render_3d.py A B D
    python3 closure/muon/render_3d.py A B C D --npz-dir closure_analysis_muon
"""
...
def main():
    ...
    parser.add_argument('--npz-dir', default=None,
                        help='Directory containing optimization_X.npz files '
                             '(default: same directory as this script)')
    args = parser.parse_args()

    npz_dir = args.npz_dir or OUT_DIR
    labels = args.labels
    print(f"Rendering 3D GIF for runs: {labels}")

    runs = {}
    for label in labels:
        path = os.path.join(npz_dir, f'optimization_{label}.npz')
