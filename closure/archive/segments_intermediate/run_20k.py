"""
Run best config at 20k segments with diagnostic plots.

Usage:
    python3 closure_analysis_full/sweeps/run_20k.py mpvmpr_20.h5
"""
...
def main():
    ...
    print(f"20k segment run — {total_steps} steps")
    ...
    print(f"Final: loss={losses[-1]:.6f}, dE_ratio={de_ratio:.3f}, "
          f"dead={dead_counts[-1]}, relocs={cumulative_relocs}")

    # --- Generate final recon signals for plotting ---
    print("\nGenerating final recon signals...")
    ...
    total_energies_arr = np.array(total_energies)

    fig, axes = plt.subplots(3, 2, figsize=(18, 20))

    # Top-left: Loss curve
    ax = axes[0, 0]
    ax.semilogy(losses_arr, 'b-', lw=1.0, alpha=0.7, label='Sobolev geomean log1p')
    ax.axvline(warmup, color='gray', ls='--', lw=1.0, alpha=0.5, label=f'Warmup={warmup}')
    ax.set_xlabel('Step', fontsize=14)
    ax.set_ylabel('Loss', fontsize=14)
    ax.set_title('Loss Convergence', fontsize=15)
    ax.legend(fontsize=11)
    ax.tick_params(labelsize=11)
    ax.grid(True, alpha=0.3)

    # Top-right: Signal comparison west_U
    ax = axes[0, 1]
    p = 3  # west_U
    if p in active_planes:
        t_sig = np.array(truth_signals[p])
        r_sig = np.array(recon_signals[p])
        t_prof = np.sum(np.abs(t_sig), axis=1)
        r_prof = np.sum(np.abs(r_sig), axis=1)
        ...
        ax.plot(wires, t_prof[wl:wh], 'b-', lw=1.5, label='Truth')
        ax.plot(wires, r_prof[wl:wh], 'r--', lw=1.5, label='Recon')
    ax.set_xlabel('Wire', fontsize=14)
    ax.set_ylabel('|Signal| (summed over time)', fontsize=14)
    ...
    ax = axes[1, 1]
    ax.plot(total_energies_arr * 1000, 'k-', lw=2.0, alpha=0.8, label='Optimizer total')
    ax.axhline(truth_total_de * 1000, color='b', ls=':', lw=1.5, alpha=0.5,
               label=f'Truth total={truth_total_de*1000:.0f} keV')
    ax.set_xlabel('Step', fontsize=14)
    ax.set_ylabel('Total Energy (keV)', fontsize=14)
    ...
    ax = axes[2, 0]
    final_de_kev = final_params[:, 3] * 1000
    alive_de = final_de_kev[final_de_kev > DEFAULTS['death_thresh'] * 1000]
    n_dead_final = len(final_de_kev) - len(alive_de)
    ...
    fig.suptitle(
        f'Full Closure [20k segments]  |  {total_steps} steps, '
        f'lr={lr}, d={decay_rate}\n'
        f'loss={losses[-1]:.6f}, dE_ratio={de_ratio:.3f}, '
        f'dead={dead_counts[-1]}, relocs={cumulative_relocs}',
        fontsize=15, fontweight='bold')
    fig.tight_layout()

    fname = os.path.join(OUT_DIR, f'run_20k_{total_steps}steps.png')
    fig.savefig(fname, dpi=150)
    plt.close(fig)
    print(f"Saved {fname}")

    # Save results
    save_path = os.path.join(OUT_DIR, f'run_20k_{total_steps}steps.npz')
    np.savez_compressed(save_path,
        losses=losses_arr,
        total_energies=total_energies_arr,
