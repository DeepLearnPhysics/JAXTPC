"""
Step count sweep with best hyperparameters found so far.
Best: lr=1.0, decay=0.999, lr_e_mult=0.003, warmup=100

Usage:
    python3 closure_analysis_full/sweeps/run_best_steps.py mpvmpr_20.h5
"""
...
def main():
    ...
    print(f"Step count sweep — best config")
    print(f"lr=1.0, decay=0.999, lr_e_mult=0.003, warmup=100")
    ...
    results = []
    for total_steps in [500, 1000, 1500, 2000, 3000]:
        cfg = dict(DEFAULTS)
        cfg['lr'] = 1.0
        cfg['decay_rate'] = 0.999
        cfg['lr_e_mult'] = 0.003
        cfg['warmup'] = 100
        cfg['noise_lr'] = 0.0
        cfg['l1'] = 0.0
        ...
        result = run_training_loop(
            grad_fn, init_params, n_seg, total_steps, 'full', cfg)
        dt = time.time() - t0
        ...
        results.append({
            ...
            'de_ratio': de_ratio,
            'dead': result['dead_counts'][-1],
            'losses': result['losses'],
            'time': dt,
        })

    print(f"\n{'='*60}")
    print(f"SUMMARY — best config step scaling")
    print(f"{'='*60}")
    print(f"{'Steps':>8} {'Loss':>12} {'dE_ratio':>10} {'Dead':>6} {'Time':>6}")
    print("-" * 45)
    for r in results:
        print(f"{r['steps']:>8} {r['loss']:>12.6f} {r['de_ratio']:>10.3f} "
              f"{r['dead']:>6d} {r['time']:>5.0f}s")

    save_path = os.path.join(OUT_DIR, 'sweep_best_steps.npz')
    save_data = {}
    for r in results:
        save_data[f"steps_{r['steps']}_losses"] = r['losses']
        save_data[f"steps_{r['steps']}_loss"] = r['loss']
        save_data[f"steps_{r['steps']}_de_ratio"] = r['de_ratio']
