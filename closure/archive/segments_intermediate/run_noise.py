"""Noise sweep at best config. Compare against known noise=0 baseline (0.026 at 1500 steps)."""
...
def main():
    ...
    results = []
    for noise_lr in [0.0, 0.1, 0.3]:
        cfg = dict(DEFAULTS)
        cfg.update({'lr': 1.0, 'decay_rate': 0.999, 'lr_e_mult': 0.003,
                    'warmup': 100, 'noise_lr': noise_lr, 'l1': 0.0,
        ...
        results.append({'noise_lr': noise_lr, 'loss': result['losses'][-1],
                        'de_ratio': de_ratio, 'dead': result['dead_counts'][-1],
                        'losses': result['losses']})

    print(f"\n{'='*50}")
    print(f"NOISE SWEEP — best config, 1500 steps")
    print(f"{'='*50}")
    print(f"{'noise_lr':>10} {'Loss':>10} {'dE_ratio':>10} {'Dead':>6}")
    print("-" * 40)
    for r in results:
        print(f"{r['noise_lr']:>10.1f} {r['loss']:>10.6f} {r['de_ratio']:>10.3f} {r['dead']:>6d}")

    save_path = os.path.join(OUT_DIR, 'sweep_noise.npz')
    save_data = {}
    for r in results:
        save_data[f"noise_{r['noise_lr']}_losses"] = r['losses']
        save_data[f"noise_{r['noise_lr']}_loss"] = r['loss']
        save_data[f"noise_{r['noise_lr']}_de_ratio"] = r['de_ratio']
