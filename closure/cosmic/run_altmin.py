#!/usr/bin/env python3
"""Block-coordinate (alternating) joint fit: solve field|tracks, then tracks|field,
repeat. The per-track geometry sub-problems are tiny/well-conditioned given the
field; the field sub-problem is the smooth shared one. Breaks the coupling that
stalled joint Adam (run_jointfull). Same realistic combined error (CRT endpoints +
scattering). Goal: reach the Fisher-predicted optimum (~1 V/cm), not ~14.
"""
...
def main():
    ...
    ap.add_argument('--track-steps', type=int, default=1500)
    ap.add_argument('--ep-lr', type=float, default=0.5)
    ap.add_argument('--c-lr', type=float, default=0.5)
    ...
    def model_one(fp, i, ep, c):
        a, b = ep[0], ep[1]; dirv = (b - a) / (jnp.linalg.norm(b - a) + 1e-6); ii = jnp.arange(NSEG, dtype=jnp.float32)
    ...
    of = optax.chain(optax.clip_by_global_norm(0.5), optax.adam(3e-4))
    oe = optax.adam(args.ep_lr); oc = optax.adam(args.c_lr)
    sf = of.init({'weights': par['weights'], 'biases': par['biases']}); se = oe.init({'ep': par['ep']}); sc = oc.init({'c': par['c']})

    @jax.jit
    def step_field(par, sf, idx):
        g = jax.grad(loss)(par, idx)
        u, sf = of.update({'weights': g['weights'], 'biases': g['biases']}, sf,
                          {'weights': par['weights'], 'biases': par['biases']})
        fp = optax.apply_updates({'weights': par['weights'], 'biases': par['biases']}, u)
        return {**fp, 'ep': par['ep'], 'c': par['c']}, sf

    @jax.jit
    def step_track(par, se, sc, idx):
        g = jax.grad(loss)(par, idx)
        ue, se = oe.update({'ep': g['ep']}, se, {'ep': par['ep']}); uc, sc = oc.update({'c': g['c']}, sc, {'c': par['c']})
        ep = optax.apply_updates({'ep': par['ep']}, ue)['ep']; c = optax.apply_updates({'c': par['c']}, uc)['c']
        return {**{'weights': par['weights'], 'biases': par['biases']}, 'ep': ep, 'c': c}, se, sc
    ...
    for r in range(args.rounds):
        for _ in range(args.field_steps):
            par, sf = step_field(par, sf, jnp.asarray(rng2.choice(M, B, replace=False)))
        for _ in range(args.track_steps):
            par, se, sc = step_track(par, se, sc, jnp.asarray(rng2.choice(M, B, replace=False)))
    ...
    print(f"[altmin M={M} ep={args.ep_sigma} sc={args.scatter}] field |E|MAE {hist[0]:.1f}->{hist[-1]:.2f} (best {min(hist):.2f}) per-round {[round(float(x),1) for x in hist]}")
